...
### #281W. Fix nightly-prefetch workflow poll bug -- wait for triggered run, not any prior `todayCompleted` [P2 -- OBSERVABILITY / CI] -- DONE 2026-08-02

**Resolution:** The workflow now records its UTC trigger epoch before POSTing, exports it to the polling step, and accepts completion only when both `todayCompleted` is true and `lastRun` is at or after the trigger time with five seconds of clock-skew allowance. A `{ started: false }` response exits without polling and reports its reason. Progress logs include both timestamps for forensics. The job timeout was raised from 5 to 35 minutes so the existing 30-minute polling window can actually complete.
...
### #292H. Phase 0: Extract a shared deterministic pricing boundary [P2 -- AI FOUNDATION] -- IMPLEMENTED 2026-08-25
...
- **Completed**:
  ...
  - ✅ `priceRoute.js` delegates deterministic calculation to `priceCoin()` while retaining HTTP validation, trusted-context derivation, auditing, redaction, safe error handling, and its legacy response contract.
  - ✅ Semiquincentennial search, key-date, mintage, and coin-variant canonicalization moved into the shared service so structured and AI callers cannot diverge.
  - ✅ Parity tests in `__tests__/pricingService.parity.test.js` (19 tests passing), including semiquincentennial, bullion precedence, denomination guard, lunar intent, and exact limitation-text behavior.
  - ✅ Focused `priceRoute`, `priceRouteValidation`, and `pricingService.parity` coverage passes: 3 suites, 71 tests.
  - ✅ Zero behavior change verified: full suite 174/174 suites, 4562/4562 tests passing.
...
- **Files**: `src/services/pricingService.js` (new boundary), `src/routes/priceRoute.js` (HTTP adapter), `__tests__/pricingService.parity.test.js`, `__tests__/priceRoute.test.js`, `docs/ARCHITECTURE.md`.
...
### #276W. `/api/price` vs `/api/pricing-batch` produce different FMV for 1/10 oz fractional Gold Eagle [P1 -- DATA-QUALITY] -- DONE 2026-06-30 (PR #219)
...
**Additional evidence (2026-06-29):** Same cross-route consistency test failed on a different sampled dataset -- `2024 Perth Lunar III Dragon Silver 2oz` (single FMV 61, batch FMV 30.5, ratio 2.0x exact match to coin weight in oz). Confirms the bug is a generic weight-multiplier divergence affecting both fractional (<1 oz, e.g. 1/10 oz Gold Eagle = 10x) AND multi-oz (>1 oz, e.g. 2 oz Silver Lunar = 2x) bullion -- not gold-specific and not fractional-specific. Surfaced during pre-commit checks for the terapeak operator codespace PR; treated as pre-existing data drift, not a new regression. Operator PR proceeds with documented BLOCK-override citing this entry. Stage 5 acceptance criterion (pinned fractional regression suite) should expand to include multi-oz bullion samples.

**Closure (2026-06-30, PR #219):**

**Root cause (NOT what the original hypothesis proposed):** The bug is in `/api/price`, not `/api/pricing-batch`. The labels in the original test-failure printout were transposed -- `priceRoute` was returning the wrong number (`meltPerOz` with no weight multiplier); `pricingBatchRoute` was returning the correct number (`meltPerOz * weight`). At 1 oz the two agree, which is why the bug only surfaced on fractional / multi-oz queries.
...
|---|---:|---:|---:|---:|
| Gold Eagle 1/10 oz | 3.05 | 30.5 (wrong) | 3.05 | 10x |
| Silver Lunar 2 oz | 61 | 30.5 (wrong) | 61 | 0.5x |
| Gold Eagle 1/4 oz | 7.625 | 30.5 (wrong) | 7.625 | 4x |
| Silver Eagle 1 oz | 30.5 | 30.5 | 30.5 | 1.0x |

**Fix (Option B -- harden priceRoute + fix test mock + pinned regression):**
...
3. `__tests__/terapeakDataIntegrity.test.js`: added 5 new `PINNED #276W` cross-route consistency tests for 1/10, 1/4, 1/2, 2 oz fractional and multi-oz bullion (Gold Eagle, Silver Eagle, Perth Lunar). Runs every test pass with strict `not.toBeNull()` guards on both routes; the regression cannot hide behind random sampling or a both-null mock breakage.

**Acceptance criteria status:**
- [x] Root cause documented (this closure block)
...
- [x] Deep review per `.github/skills/workflow/SKILL.md`: 0 S1/S2 findings; 2 S3/2 S4 addressed in same PR
...
- Deep-review S4 items (deferred as polish): consolidate 3x `parseDescription` calls in `priceRoute.js`; replace `eval(w)` in test-mock helper (pre-existing).
- Deep-review Step 9 workflow gap: codify "single header, edit in place" as the closure convention in `.github/skills/workflow/SKILL.md` (surfaced by this PR's own convention violation -- see WASTE-LEDGER INC-014).
