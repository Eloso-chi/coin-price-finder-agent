...
Structured `coinData` may include `composition` (`silver`, `gold`, `platinum`, or `palladium`), `label: "Privy"`, and an optional `variantDetail` containing the specific privy name or mark. `variantDetail` is limited to 50 letters, numbers, spaces, and `._+=-` characters; `+` or `-` cannot begin a token because provider search operators are not valid privy identity. The same validation runs before deterministic or LLM-backed AI routing. Specific privy details are included in provider keywords and become a hard sold-comp eligibility condition; a request for `EMC2` cannot use a different privy or generic Reverse Proof comp.
