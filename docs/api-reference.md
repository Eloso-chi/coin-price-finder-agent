...
Allowed caller context is server-derived `{ audience, isAdmin }`; request bodies and model arguments cannot set identity, admin state, audience, secrets, provider configuration, or arbitrary function names. The orchestrator allows one tool call per turn and at most three tool turns. Tool results are validated and returned to the LLM for explanation; numerical explanations without deterministic evidence are rejected. Registry timeouts are enforced before a tool result is accepted. Focused coverage is in `__tests__/aiToolRegistry.test.js`, `__tests__/aiOrchestratorService.test.js`, `__tests__/aiPriceRoute.integration.test.js`, `__tests__/aiPriceRoute.llm.test.js`, and `__tests__/llmProviderAdapter.test.js`.
