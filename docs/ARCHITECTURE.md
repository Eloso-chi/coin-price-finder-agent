...
**Exact privy isolation (#311H):** Privy is a requestable variant family. Structured Entry captures an optional bounded `variantDetail` such as `EMC2`; `pricingService` carries it into provider keywords and the expected-comp contract. `scoreMatch()` ranks exact normalized detail matches and `applyFilters()` hard-removes every other mark or generic listing. A requested proof or Reverse Proof privy remains in its corresponding strike pool instead of being forced into raw specialty comps.
