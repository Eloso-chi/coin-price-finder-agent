...
**Registry-backed special marks (#312H):** Special marks are orthogonal to finish, mintmark, design type, security features, and certification. `specialMarksRegistry.js` stores provenance-backed issue constraints and aliases under stable `markId` values. Structured Entry obtains applicable choices from `GET /api/special-marks`; exact selections carry `specialMarkMode: "exact"` plus `specialMarks`, while explicit standard issues use `specialMarkMode: "standard"` and unlisted marks remain unknown through the bounded legacy `variantDetail` path. `pricingService` resolves the mark into canonical product identity before provider lookup. `scoreMatch()` and `applyFilters()` use registry aliases for exact cohorts; explicit standard requests reject recognized marks. A mark never determines the strike pool: Proof and Reverse Proof remain strictly isolated.
