...
**Registry-backed special marks (#312H, #313H):** Special marks are orthogonal to finish, mintmark, design type, security features, and certification. `specialMarksRegistry.js` stores provenance-backed issue constraints and aliases under stable `markId` values, plus immutable `issueId`, verification state, and issue-scoped mintage metadata. Structured Entry obtains applicable choices from `GET /api/special-marks`; exact selections carry `specialMarkMode: "exact"` plus `specialMarks`, while explicit standard issues use `specialMarkMode: "standard"` and unlisted marks remain unknown through the bounded legacy `variantDetail` path. Metal-specific Maple Leaf names deterministically resolve silver or gold, while the generic Maple Leaf remains ambiguous. Multiple applicable registered marks return a clarification requirement instead of a blended valuation. `pricingService` resolves the mark into canonical product identity before provider lookup. `scoreMatch()` and `applyFilters()` use registry aliases for exact cohorts; explicit standard requests reject any title positively declaring a privy. Exact marks bypass cross-year composite construction. A mark never determines the strike pool: Proof and Reverse Proof remain strictly isolated. The broader Silver Maple Leaf census is quarantined in `data/research/` and is not imported by runtime pricing code.
...
**Canonical product identity:** `productIdentityResolver.js` emits parser version `2.0.0` with series, year, mint, metal, nominal weight, weight evidence, grade, finish, designation, `specialMarkMode`, `specialMarks`, and target pool. Registered marks are accepted only when their program/year/metal/weight/finish/mint applicability matches; conflicting structured mark identity returns `AMBIGUOUS_PRODUCT_IDENTITY` before provider calls. Special marks do not alter pool derivation. Price Discovery, pricing batch, bulk evaluation, deterministic AI, direct comp filtering, composite filtering, and Terapeak import consume this contract. Structured request fields outrank compatible text evidence; conflicting or multi-weight evidence is rejected. Nominal and metric weight evidence within 5% is equivalent, while distinct fractional products remain separate. A generic Maple Leaf name is compatible with a parser's metal-qualified Maple Leaf series only when no explicit series contradiction exists; explicit Silver-vs-Gold series and metal conflicts remain ambiguous.
...
When a year-specific Terapeak pool remains below three comps, `fetchSoldComps()` may search the same type, weight, finish, designation, and grade pool across +/-3 years. It uses the cohort only when the deduplicated pool reaches five comps. Every direct-year and cohort candidate admitted to a composite must state exactly one eligible year and at least one detectable weight, and every detected weight must match the target within 5%; unknown- or mixed-weight listings cannot satisfy the minimum. `computeValuation()` labels this `cross-year-composite`, records only the qualifying bounded years and counts in `gradePool.compositeBasis`, and caps confidence at 30 when target population is unavailable or 35 when available. A known target population below 50 removes cohort comps and keeps the direct-year result. Batch, bulk, AI, UI, and CSV consumers preserve and disclose this provenance.
