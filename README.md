...
| Tab | Description |
|-----|-------------|
| **Price Discovery** | Main search -- two sub-modes: Coin (structured or quick-search entry) and Bar/Bullion. Structured Coin mode offers Auto/Silver/Gold/Platinum/Palladium metal selection for multi-metal world-bullion series and retains the conditional "Silver" checkbox for post-1992 US proof denominations. Canadian Silver Maple Leaf and Canadian Gold Maple Leaf names set deterministic metal context. The Special mark / privy selector loads registry-backed choices filtered by series, year, metal, weight, mint, and finish; users can explicitly select a standard issue or describe an unlisted mark, which remains unverified. Multiple applicable marks require an exact choice. Exact marked comps remain isolated within the requested Proof or Reverse Proof pool and never use cross-year composites. Bar mode has a dynamic series dropdown populated from `GET /api/bar-price/options`. Submits to `/api/price` or `/api/bar-price` and renders FMV, confidence, decisions, metadata, comps, and explicit low-data warnings. |
...
### Deterministic Terapeak startup operator (#168, June 21, 2026)
...
- Upload-mode default behavior is now split by entrypoint:
  ...
  - Set `UPLOAD_MODE` explicitly when you need non-default behavior.
...
- **Grade-type pool split** -- `classifyGradeType()` in `ebayService.js` and `terapeakService.js` classifies comps into three pools: `graded` (slabbed/certified), `proof` (slabbed or unslabbed proof coins), or `raw` (BU/circulated). `classifyGradeType()` checks `conditionId=2000` comps for proof regex in title, so slabbed proofs (PCGS PR69 DCAM) correctly land in the proof pool instead of graded (#182). The valuation engine selects the appropriate pool: proof if grade is "Proof"/"PF"/"PR", graded if any other grade is specified, raw otherwise. For proof queries, the proof pool is used unconditionally regardless of count (1-2 comps flags `lowData` + reduced confidence; never falls back to BU comps -- #184). **Pool isolation is now strict for all three pools** -- raw queries use only raw comps (#270W Option #4 / PR #186) and graded queries use only graded comps (#272W); there is no silent fallback to all-comps when the preferred pool is thin. When a graded query has zero graded comps AND no PCGS price guide AND no Greysheet anchor available, the engine returns a last-resort raw blend only if at least 10 raw sold comps exist (the tightened #176 V2 fallback); otherwise FMV is null with `dataSource.label = 'guide-only'` (guide present but no comps) or `'metal-only'` (bullion fallback). Proof detection uses `/\bproof\b(?![\s-]*like)/i` to avoid misclassifying "proof-like" (PL) coins. The `gradePool` response field now includes `wantsProof`, `proofCount`, and the 3-way `usedPool` label.
...
- **YearMismatch relaxation for bullion generics (#165)** -- Generic Terapeak datasets (e.g. "China 1 oz Silver Panda") span all years. Previously the `yearMismatch` filter dropped most comps when pricing a specific year from a generic dataset, leaving only 5-7 comps with low confidence. Now: if a bullion coin's expected year is absent from the dataset name, the yearMismatch filter is skipped entirely. Year-specific datasets and non-bullion coins retain strict year filtering. Result: 2020 Panda went from 5 comps/conf 27 to 119 comps/conf 78.
- **BU grade pool fix for Lot Evaluator (#162)** -- `bulkEvaluateService.js` now nulls out `effectiveGrade`/`effectiveGradeNum` when a bullion coin's grade came from a BU-term detection (e.g. "BU" in description). This prevents BU coins from being filtered into the graded pool and losing comps. Matches the existing behavior in `priceRoute.js` and `pricingBatchRoute.js`.
...
- **`refresh-stale.sh` updated** -- now passes `--refresh --max-age "$STALE_DAYS"` instead of `--resume`, correctly re-collecting coins that have old data.
...
- **Cosmos DB write-through** -- `aggregationMeta` is now persisted to the `terapeak-sold` Cosmos DB container alongside the comp data. On server startup, `hydrateMetaFromCosmos()` reads back all stored meta from Cosmos and merges it into the in-memory dataset map, so deep-pagination markers survive server restarts even if the local `terapeak_sold.json` cache is cleared.
- **deepAt inference** -- `importComps()` now automatically infers `deepAt` for datasets that arrive with >=100 comps but no explicit `deepAt` marker. This handles legacy deep-paginated datasets that were collected before aggregationMeta tracking was added.
- **Gold exclusion from deep pagination** -- `sales-aggregator.py` now excludes gold coins (regex `\bgold\b`) from deep pagination candidates. Gold coins have high per-item values that skew aggregation priority but are rarely worth the quota cost for deep enrichment. The exclusion applies both in `get_candidates_from_server()` and the dashboard display.
