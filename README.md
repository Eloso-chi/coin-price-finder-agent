...
- **Exclusion operator support** -- `parseDescription()` now honors eBay-style exclusion operators (`-proof`, `-gold`, `-W`, `-burnished`, etc.) in keyword strings. Exclusion tokens are stripped from the parsed fields and passed through as negative keywords to eBay queries, preventing unwanted variants from contaminating comps.
