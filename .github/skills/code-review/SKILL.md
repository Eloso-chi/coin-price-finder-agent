...
| Disk writes under test | `docs/WASTE-LEDGER.md` INC-002 | Tests can reach real repository/operator state because a persistence path lacks a `NODE_ENV === 'test'` no-op or injected mocked/temp path; debounce timers write after teardown |
